...
The component to be rendered in the slot. The component should be an Astro component. However you can use any component that can be rendered by Astro but imported component should be .astro files.
...
import { ClubsFunctionGetSlots } from '@devprotocol/clubs-core'
...
export const getSlots: ClubsFunctionGetSlots = async (
	...
	const [path] = paths
	return factory === 'admin' && path === 'example'
		? [
				{
					// highlight-start
					...
					props: {
						ExampleComponent: {
							config,
							label: `Add 'Example' to the menu`,
							link: { display: 'Community', path: '/example' } as ExamLink,
						},
						// highlight-end
					},
				},
