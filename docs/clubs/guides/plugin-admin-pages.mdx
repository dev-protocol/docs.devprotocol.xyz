...
The library [clubs-core](https://github.com/dev-protocol/clubs-core/blob/main/src/types.ts) has helpers and utility functions that can be used to update a club easily and efficiently. Plugin options store necessary information about a club's plugin and has the type `ClubsPluginOptions`(<Types />). The helper function [setOptions](https://github.com/dev-protocol/clubs-core/blob/main/src/events.ts#L19) enables updating plugin options.
