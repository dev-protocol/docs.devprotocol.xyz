...
The helper function [setOptions](https://github.com/dev-protocol/clubs-core/blob/main/src/events.ts#L19) enables updating plugin options. Using `setOptions` will trigger the DOM(browser managed local instance of a club) to update plugin options locally in the browser's memory.

The [`setOptions`](https://github.com/dev-protocol/clubs-core/blob/main/src/events.ts#L19) helper accepts two paramteres: the new plugin options and your clubs configuration plugin index.

:::note
The `setOptions` helper function updates the entire state of a plugin.

If you have to retain any portion of the plugin state, you will have to inject it in the new state and then use `setOptions`.
:::
